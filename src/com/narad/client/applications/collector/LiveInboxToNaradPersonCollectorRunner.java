...
import com.narad.client.applications.collector.queue.FolderListProfileQueue;
...
import com.narad.client.applications.collector.writer.NaradPersonWriter;
import com.narad.client.applications.collector.writer.TwitterToNaradProfileWriter;

public class LiveInboxToNaradPersonCollectorRunner {
	private static final Logger logger = LoggerFactory.getLogger(LiveInboxToNaradPersonCollectorRunner.class);
	...
	public static void main(String[] args) throws Exception {
		LiveInboxToNaradPersonCollectorRunner liveInboxRunner = new LiveInboxToNaradPersonCollectorRunner(
				"D:\\development\\narad\\response_dump", "liveInbox", "D:\\development\\narad\\response_dump");
		liveInboxRunner.runCollectorAsync();
	}

	public LiveInboxToNaradPersonCollectorRunner(String baseFilePath, String network, String filePath) throws Exception {
		super();
		...
		} else if (network.equals("liveInbox")) {
			this.reader = new LiveInboxReader(baseFilePath);
			this.writer = new NaradPersonWriter(null, naradUrl);
		} else {
		...
		if (filePath == null) {
			...
			profileQueue = new FolderListProfileQueue(filePath + File.separator + network);
		}
	}
			...
			try {
				...
				if (!friendExists && (userExists || fetchUser != null)) {
					...
					if (fetchFriends != null) {
						writer.saveFriends(fetchUser, fetchFriends);
